...
actions = ['sq', 'oneleg' , 'jump']
seq_length = 30
secs_for_action = 50

model_keras = load_model('./models/model.keras')
...
pose_est_path = os.path.join(model_path,"human-pose-estimation-0001.xml")
...
model = core.read_model(pose_est_path)
...
# def import_joint(where):
#     joint = np.zeros((18, 2))
#     for j, lm in enumerate(pose):
#         joint[j] = [lm[0], lm[1]]
    
#     head = joint[2]
#     left_shoulder = joint[5]
#     right_shoulder = joint[6]

#     where2 = where

#     return where2

# abc = import_joint(head)
...
while cap.isOpened():
    ...
    if poses is not None:
        for pose in poses:
            ...
            y_pred = model_keras.predict(input_data).squeeze()
